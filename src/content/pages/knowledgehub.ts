...
export const knowledgehub: ProjectPageData = {
  ...
  problem: {
    body: "I keep context repos on GitHub: markdown notes on my projects, investing research, and things I want an agent to know about me. Any agent can already read them from a chat, but there was no good way to edit them. The first version of this server was a passthrough to GitHub's own MCP tools. Those tools know nothing about markdown, and writing through GitHub's contents API makes a separate commit for every file, so one logical change came out as a trail of commits. I deleted the passthrough and built the editing layer myself. The server gives the agent seven tools that only deal in markdown, and every change, however many files it touches, ends up as exactly one commit. If any part of a batch fails, nothing is written. It runs as one Docker container with no database and no runtime dependencies, just Node built-ins.",
    media: [
      {
        kind: 'pre',
        pre: `the pieces
