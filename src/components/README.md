...
- `ProjectPage/` — `ProjectPage`: a project's own page as one terminal window running `cat README.md` — header (name, dates, lede, concepts/stack/remote rows), problem, features (`SubSection[]`, media rendered before body), difficulties (`SubSection[]`, body before media), `$ cd ..` back. Data: `ProjectPageData`. Story: `Pages/Project`.
